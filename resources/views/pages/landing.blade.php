...
@section('title', 'Blog Masterprima | Berita Seputar Kedinasan, CPNS, PPPK, & UTBK')
...
<div class="flex flex-col px-4 md:px-10 lg:px-14 mt-10">
    ...
    <div class="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-12 gap-5">
        ...
        <a href="{{ route('news.show', $new->slug) }}"
            ...
            <img src="{{ asset('storage/' . $new->thumbnail) }}" alt="berita2" class="rounded-xl md:max-h-48" style="width: 250px; object-fit: cover;">
            <div class="mt-2 md:mt-0">
                <p class="font-semibold text-lg">{{ $new->title }}</p>
                <p class="text-slate-400 mt-3 text-sm font-normal">
                    {!! \Str::limit($new->content, 50) !!}
                </p>
            </div>
        </a>
        @endforeach
    ...
    <div class="grid grid-cols-1  sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
        ...
        <a href="{{ route('author.show', $author->username) }}">
            <div
                ...
                <p class="text-slate-400">{{ $author->news->count() }} Artikel</p>
            </div>
        </a>
        @endforeach
    </div>
